## 🙋🏻‍♀️ 열번째 비대면 수업

- **배운 내용**

    검색 관련 명령어

    grep 명령어

    find 명령어

    기초 문제 풀이

---

# 내용 정리

# 검색 관련 명령어

# 단원목표

- grep 명령어

    파일의 컨텐츠를 들여다 보는 경우

- find 명령어

    파일명, 각각의 속성을 대상으로 검색하는 경우

---

# grep 명령어


- **grep 명령어**

    ```bash
    # grep OPTIONS PATTERN ARG1
    ```

    **OPTIONS : -l, -n, -I, -v, -w**

    **PATTERN : * . ^root root$ [abv] [^a]**

```bash
# grep pattern [file path]   < 고정된 파일에서 많이 사용 가능
# CMD | grep pattern         < 실시간 자료, 명령어로 출력 결과를 봐야하는 상태인 경우 사용되는 명령어 검색 형식
# ps –ef | grep pattern 
# rpm –qa | grep pattern 
# cat /etc/passwd | grep root
# cat /etc/group | grep root
# cat /var/log/messages | egrep '(warn|err|crti|alert|emerg)'
```

---

## grep 명령어

하나 이상의 패턴과 일치하는 행을 선택하여 주어진 파일 [및 디렉토리]를 탐색

파일 내에서 특정한 패턴을 검색하여 그 패턴을 포함하는 모든 줄을 화면에 출력하는 명령어. 파일 내에 특정한 패턴을 찾기 위해 많이 사용한다. 찾으려고 하는 패턴을 정규식(**g/re/p, Globally/Regular Expression/Print**)이라고 표현한다. grep 명령어의 약자에서 볼 수 있듯이 많은 패턴을 제공하고 있다.

[명령어 형식]

```bash
# grep OPTIONS PATTERN file1
```

(기본 사용법)

```bash
# grep root /etc/passwd    (# cat /etc/passwd | grep root)
# CMD | grep root 
# cat /etc/passwd | grep root 
# rpm -qa | grep sshd 
# ps -ef | grep socket 
```

(옵션 사용법) "# grep OPTIONS PATTERN file1"

```bash
# grep -l root /etc/hosts /etc/passwd /etc/group /* 여러 파일 중 검색 문자열 존재 파일 출력 */
# grep -n root /etc/group  /* -n: number line, 파일에서 root 문자열을 검색하고 라인 번호도 같이 출력 # cat -n /etc/passwd | grep root */ 
# grep -v root /etc/passwd  /* -v: inverse, 파일에서 검색 문자열을 제외하고 나머지 출력 */
# grep -i root /etc/passwd  /* -i: ignore case, 파일에서 검색 문자열의 대소문자를 구분하지 않음 */
# grep -w root file1        /* -w: word , 단어의 일부분이 아닌 단어의 전체가 일치하는 경우 출력 */
# grep --color root /etc/passwd
```

**[명령어 옵션]**

**옵션 : 설명**

**-l** : (-l : list files) 패턴이 있는 파일이름만을 출력한다. (여러 파일이 존재하고 이 파일들중 검색하는 패턴이 존재하는 파일만 찾고싶을 때)
-n : (-n : number line) 패턴을 포함하는 줄을 출력할 때 줄번호와 함께 출력한다.
**-v** : (-v : inVerse, except) 패턴을 포함하는 줄을 제외하고 출력한다. (패턴에 해당하는 라인을 검색에서 제외)
-c : (-c : count) 패턴을 찾은 줄의 수를 출력한다.
**-i** : (-i : ignore case, 대문자/소문자) 패턴을 찾을 때 대소문자를 구분하지 않는다.

**-w** : 패턴에 정확하게 일치하는 것만 검색

-E : egrep과 똑같이 작동한다. 한번에 여러 개의 패턴을 검색하는 경우 사용한다.

(패턴 사용법) "# grep OPTIONS PATTERN file1"

```bash
*       # grep 'ro*t' /etc/passwd       모든문자를 포함(영문자,숫자) 한 자 이상 
.       # grep 'no...y' /etc/passwd     (.에 하나의 영문자나 숫자를 치환)
^root   # grep '^root' /etc/passwd      라인의 시작이 root인 라인을 검색
root$   # grep 'root$' /etc/group       라인의 끝이 root로 끝나는 라인을 검색
[abc]   # grep 'user0[123]' /etc/passwd user0으로 시작하여 1이나 2나,3으로 끝나는 문자열
```

앵커 : ^ $

메타문자

와일드 카드 : *

### [EX1] grep 명령어 옵션 실습

```bash
# useradd FEDORA
# passwd FEDORA 
-> 사용자 암호 입력

# grep fedora /etc/passwd    (# cat /etc/passwd | grep fedora)
fedora:x:1001:1001::/home/fedora:/bin/bash
# grep FEDORA /etc/passwd
FEDORA:x:1004:1004::/home/FEDORA:/bin/bash

리눅스는 대소문자를 구별하기 때문에 검색을 하는 경우에 대소문자를 정확히 구별하여 패턴 작성을 해야 한다.
하지만 이것을 무시하고 검색을 해야 하는 경우에는 -i 옵션을 사용해준다.
# grep -i fedora /etc/passwd
fedora:x:1001:1001::/home/fedora:/bin/bash
FEDORA:x:1004:1004::/home/FEDORA:/bin/bash
```

(실무 예) 환경 파일에 등록

```bash
# alias grep='grep -i'          => $HOME/.bashrc 
# alias grep='grep -i --color'  => $HOME/.bashrc < 검색된 단어에 색상을 입혀 가독성을 높여준다 >

# vi ~/.bashrc 
alias grep='grep -i --color'
# . ~/.bashrc 
# alias grep
```

```bash
# cat -n /etc/passwd | grep fedora
    48	fedora:x:1002:1002::/home/fedora:/bin/bash
# grep -n fedora /etc/passwd
48:fedora:x:1002:1002::/home/fedora:/bin/bash
# grep -n root /etc/group
1:root:x:0:
# grep -l root /etc/group /etc/passwd /etc/hosts
/etc/group
/etc/passwd
# vi file.txt
root hello
roothello
testroottest
helloroot
# grep -w root file.txt
root hello
```

### [EX2] grep 명령어 패턴 실습

```bash
# nmcli con show
NAME   UUID                                  TYPE      DEVICE 
ens33  7da16185-c5c7-47de-8e44-07c3276d5006  ethernet  ens33

# grep -lr ens33 /etc
/etc/sysconfig/network-scripts/ifcfg-ens33

# cat /var/log/messages | grep -i dec | head -5
Aug 21 14:33:19 server1 kernel: Linux version 4.18.0-193.el8.x86_64 (mockbuild@kbuilder.bsys.centos.org) (gcc version 8.3.1 20191121 (Red Hat 8.3.1-5) (GCC)) #1 SMP Fri May 8 10:59:10 UTC 2020
Aug 21 14:33:19 server1 kernel: Command line: BOOT_IMAGE=(hd0,msdos1)/vmlinuz-4.18.0-193.el8.x86_64 root=/dev/mapper/cl-root ro resume=/dev/mapper/cl-swap rd.lvm.lv=cl/root rd.lvm.lv=cl/swap rhgb quiet
Aug 21 14:33:19 server1 kernel: Disabled fast string operations
Aug 21 14:33:19 server1 kernel: x86/fpu: Supporting XSAVE feature 0x001: 'x87 floating point registers'
Aug 21 14:33:19 server1 kernel: x86/fpu: Supporting XSAVE feature 0x002: 'SSE registers'
# cat /var/log/messages | grep -i 'dec 26'
# cat /var/log/messages | grep -i ^dec | tail -5
# ps
PID TTY          TIME CMD
13929 pts/2    00:00:00 bash
# ps -ef | grep 13929
root     13929 13927  0 14:00 pts/2    00:00:00 -bash
# ps -ef | grep bash
root      7117  7115  0 00:53 pts/3    00:00:00 -bash
root      7364  7117  0 01:47 pts/3    00:00:00 -bash
root     27308 27302  0 Jan26 pts/2    00:00:00 bash
# rpm -qa | grep postfix
postfix-8.13.8-2.el5
postfix-cf-8.13.8-2.el5

		# rpm -q postfix 
		# rpm -qa postfix  
		# rpm -qa | grep postfix
```

### [EX3] 파일내의 특정 패턴 여러개 검색하기

```bash
grep의 태생적 한계 -> 하나이상의 패턴 검색이 불가능
■ egrep(Extended grep) CMD /* 동시에 여러개의 패턴을 검색하는 경우에 사용 &/
# cat /var/log/messages | egrep -i '(warn|err|crit|alert|emerg)' 
# cat /etc/passwd | egrep -i '(root|fedora)'
root:x:0:0:root:/root:/bin/bash
operator:x:11:0:operator:/root:/sbin/nologin
fedora:x:1001:1001::/home/fedora:/bin/bash
FEDORA:x:1002:1002::/home/FEDORA:/bin/bash
# cat file1 | grep -E 'http|telnet|ssh'
-E : egrep과 똑같이 작동한다.
■ fgrep(Fixed grep) CMD
# fgrep '^root' file1
```

**fgrep** : 앵커 등 특수문자를 포함하여 검색한다. 특수문자를 같이 사용해서 패턴으로 인식해야 하는 경우에 사용되는 grep

```bash
# egrep "fedora|root" /etc/passwd
root:x:0:0:root:/root:/bin/bash
operator:x:11:0:operator:/root:/sbin/nologin
fedora:x:1001:1001::/home/fedora:/bin/bash
```

(실무 예) 로그 파일에서 에러 메세지 검색

```bash
# cat /var/log/messages | egrep –i '(warn|err|crit|alert|emerg)'
```

```bash
# egrep -v "fedora|user01" /etc/passwd    (# egrep -v "(fedora|user01)" /etc/passwd)
root:x:0:0:root:/root:/bin/bash
bin:x:1:1:bin:/bin:/sbin/nologin
daemon:x:2:2:daemon:/sbin:/sbin/nologin
..... (중략) .....
```

(실무) 특정한 단어를 제외하고 검색

```bash
# ps –ef | grep sshd | grep –v grep 

grep : 강력한 패턴 매칭 템플릿을 정의하기 위해 "정규 표현식"을 사용할 수 있다.  
egrep[옵션] "패턴|패턴|..." [대상파일들] : 확장된 정규 표현식을 사용하며, 찾아낼 패턴을 여러개 지정할 수 있다. 
'|'기호는 불린 연산자 "OR"에 해당하므로, 정해진 패턴들에 포함되는 모든 라인을 보여준다.  
fgrep[옵션] 패턴 [대상파일들] : 패턴과 정확히 일치하는 것만을 찾아 준다.
```

# find 명령어


검색을 하는 명령어

inode의 정보를 바탕으로 검색을 하는 명령어

```bash
# find / -name core –type f
# find / -user user-1 –group class1
# find / -mtime [-7|7|+7]
# find / -perm [755|-755]
# find / -size [-300M|300M|+300M]
# find / -name core –type f –exec rm –f {} \;
# find / -name core –type -ls
# find / -name core –type f –ok rm {} \;
# find /Log_Dir –name "*.log" –type f \
> -mtime +30 –exec rm 0rf {} \;
# find /Log_Dir –mtime -2 –size +1G –type
```

---

## find 명령어

디렉토리안에서 원하는 파일을 찾고자 할 때 사용하는 명령어이다. find 명령 다음에 시작 디렉토리를 정해 주고 찾고자하는 파일 이름 앞에 옵션을 주면 된다.

[명령어 형식]

```bash
# find [검색시작위치] [옵션1] [인자값1] [옵션2] [인자값2] ...
```

[명령어 옵션]

**옵션 : 설명**

**-name :** 파일 이름을 기준으로 검색

**-iname: 대소문자를 구별하지 않고 파일 이름을 기준으로 검색**

**-perm :** 파일 권한을 기준으로 검색한다.

**-type :** 파일의 종류를 기준으로 검색

b : 블록 파일

c : 문자

**d : 디렉토리**

**f : 파일**

**l : 링크**

s : 소켓

**-size :** 파일의 크기를 기준으로 검색

+n : n보다 크다

-n : n보다 작다

n : n이다

b : 512-byte

**c : Bytes (Character = Byte)**

**k : Kilo Byte**

**M : Mega Bytes**

**G : Giga Bytes**

w : 2-byte

-links : 링크의 개수를 기준으로 검색

-user : 사용자 ID를 기준으로 검색

-group: 그룹 ID를 기준으로 검색

-atime : 특정 기간 이상 접근하지 않은 파일을 기준으로 검색

-mtime : 특정 기간 이상 수정되지 않은 파일을 기준으로 검색

-inode : number 지정된 inode 번호와 파일을 찾는다.

-print : 표준출력으로 검색된 파일 출력: GNU는 디펄트, Unix는 필수 입력

-exec command {} ₩; :찾은 각 파일에 대해 지정된 명령을 실행

-ok command {} \; : 실행여부(실행 되어 있는지 아닌지)를 사용자에게 확인 후 명령을 실행

```bash
------------------------------------------------------------------------------
(형식1) # find / -name core -type [f|d]    (# find / -name "*oracle*" -type f)
(형식2) # find / -user user01 -group class1 
(형식3) # find / -mtime [-7|7|+7] 
(형식4) # find / -perm [-755|755] 
(형식5) # find / -size [-300M|300M|+300M]
(형식6) # find / -name core -type f -exec rm {} \;
------------------------------------------------------------------------------
```

### [EX1] 파일 이름 검색(예: # find / -name core –type f)

```bash
/test -+- dir1 - file1 
			 |
			 +- dir2 - file3
		   |
			 +- file1
			 |
			 +- file2
```

```bash
And 가 아닌 OR 연산이 필요한 경우
# find /var -type f -or -type d | wc -l
```

```bash
# cd /test 
# find . -name file1
./dir1/file1
./file1
-> "."은 현디렉토리, 이름이 file1인 파일 또는 디렉토리를 찾아라 디렉토리이던 파일이던 type과 상관없이 모두 찾는 것
# find . -name file1 -type f
./dir1/file1
./file1
# find /var -name "*.log" -type f
/var/log/audit/audit.log
/var/log/sssd/sssd.log
/var/log/sssd/sssd_implicit_files.log
/var/log/sssd/sssd_nss.log
/var/log/sssd/sssd_kcm.log
/var/log/tuned/tuned.log
..... (중략) .....
# find /var -name "*log*" -type f
-> log가 들어가는 모든 파일, *log -> 로그로 끝나는 파일, log* -> 로그로 시작하는 파일
```

### [EX2] 사용자/그룹 검색(예: # find / -user user01 -group class1)

```bash
# find /home -user fedora -group fedora
/home/fedora
/home/fedora/top.seceret.mail
/home/fedora/.bashrc
..... (중략) .....
# find /home -user fedora -group fedora -ls 
-> 출력 결과 확인
```

### [EX3] 날짜 검색(예: # find / -mtime 7)

```bash
# cd dir2 
# touch file1 file2 file3 file4 file5 file6 

# date 08191111
Thu Aug 19 11:11:00 KST 2010

참고 - vmware에서 실습을 진행하는 경우 NTP로 인하여 , VMware tools로 인하여 시간이 계속 변경되기 때문에 
터미널을 하나 더 실행하여 다음 명령어로 현재 시간을 확인하며 실습을 진행한다.
# watch -n date
# while : ; do date 08191111; sleep 1; echo "date resetting"; done
```

[참고] 파일 속성 정보

```bash
     file6      file5       file4        file3      file2      file1
  -----+----------+------------+----------+----------+----------+---------+   
     0813        0814         0815       0816      0817        0818      0819
```

```bash
# touch -t 08181111 file1 
# touch -t 08171111 file2 
# touch -t 08161111 file3 
# touch -t 08151111 file4 
# touch -t 08141111 file5 
# touch -t 08131111 file6
# ls -l file*
-rw-r--r-- 1 root root 0 Aug 18 11:11 file1
-rw-r--r-- 1 root root 0 Aug 17 11:11 file2
-rw-r--r-- 1 root root 0 Aug 16 11:11 file3
-rw-r--r-- 1 root root 0 Aug 15 11:11 file4
-rw-r--r-- 1 root root 0 Aug 14 11:11 file5
-rw-r--r-- 1 root root 0 Aug 13 11:11 file6
# find . -mtime 3 -type f /* 수정 날짜가 3일전인 파일 */
./file3
# find . -mtime -3 -type f /* 수정한 날짜가 3일이 안된 파일 */
./file3
./file2
./file1
# find . -mtime +3 -type f /* 수정한 날짜가 3일이 지난 파일 */
./file6
./file5
./file4
```

### [EX4] 퍼미션 검색(예: # find / -perm 755 -type f)

```bash
# cd /test 
# mkdir dir3 
# cd dir3 
# touch file1 file2 file3 file4 file5 file6 file7 file8 

# chmod 000 file1  (---)
# chmod 100 file2  (--x)
# chmod 200 file3  (-w-)
# chmod 300 file4  (-wx)
# chmod 400 file5  (r--)
# chmod 500 file6  (r-x)
# chmod 600 file7  (rw-)
# chmod 700 file8  (rwx)

# ls -l file*
---------- 1 root root3043 3월 18 14:54 file1
---x------ 1 root root362131 3월 18 14:52 file2
--w------- 1 root root  0 3월 18 14:52 file3
--wx------ 1 root root  0 3월 18 14:52 file4
-r-------- 1 root root  0 3월 18 14:52 file5
-r-x------ 1 root root 0 3월 18 14:52 file6
-rw------- 1 root root  0 3월 18 14:52 file7
-rwx------ 1 root root 0 3월 18 14:52 file8
# find . -perm 600 -type f –ls 
-> file7

# find . -perm -400 -type f -ls 
-> file5, file6, file7, file8

SETUID SETGID 동시에 검색
# find /  -perm -2000 -o -perm -4000
> 아래는 에러 제거
# find / -perm -4000 -not -path "/proc/*"
# find / -perm -4000  2> /dev/null

숫자 앞에 -를 붙일 시 해당 권한보다 높은 권한을 가진 모든 파일을 찾는다.
# find . -perm 4000
	> 4000 권한을 가진 파일 검색
# find . -perm -4000
	> 000 값에 들어있는 퍼미션은 무시
```

### [EX5] 파일 크기 검색(예: # find / -size 50k -type f)

사용자: 디스크가 full이 차있다 → 최근에 생긴 파일 → 용량이 큰 파일

```bash
# find / -size +500m -mtime -1
# cp /etc/passwd file10 
# cp /etc/services file11 
# ls -l 

* file1-8   0    Bytes
* file10  3228   Bytes
* file11  692252 Bytes

# find . -size 2490c –type f    /* 정확이 일치되는 용량만 검색 */
-> file10

# find . -size -1980c –type f   /* 일치되는 것 제외한 파일 크기가 2027bytes 미만 */
-> file1-8

# find . -size +1980c –type f   /* 파일 크기가 2027bytes 보다 큰것 */
-> file11
```

후처리 명령어를 바로 입력하여 실행 할 수 있다.

### [EX6] 디렉토리안에 특정한 패턴을 가진 파일들을 삭제(# find / -name file -type f -exec rm {} \;)

-exec 

find [path][option] -exec[명령어 체계]

find로 검색된 파일, 디렉토리에 대하여 한번에 무언가 작업을 이루기 위해서

[형식]

```bash
-exec CMD {} \;
			chmod u-s {} \;       /*user에서 setuid를 빼겠다*/
			chmod u-a {file} \;
cp, chown, source-target

# find . -perm -4000 -exec chmod -v u-s {} \;
mode of './file2' changed from 4500 (r-s------) to 0500 (r-x------)
mode of './file4' changed from 4705 (rws---r-x) to 0705 (rwx---r-x)
mode of './file5' changed from 7777 (rwsrwsrwt) to 3777 (rwxrwsrwt)

cpetc 디렉토리에서 링크 파일만 찾아 삭제
#find . -type l -exec rm -f {} \;
cpetc 디렉토리에서 파일만 찾아 /test/dir1으로 이동
# find . -type f -exec mv -vf {} /test/dir1 \;
```

```bash
# cd /test
# find . -name file1 -type f /* 파일의 이름이 file1인 것 */
./file1
./dir2/file1
```

```bash
# find . -name file1 -type f -ls 
# find . -name file1 -type f -exec chown -v user01 {} \; 
# find . -name file1 -type f -ls 

# find . -name file1 -type f -exec chmod -v 640 {} \; 
# find . -name file1 -type f -ls 

# find . -name file1 -type f -exec rm -fv {} \; 
# find . -name file1 –type f

/proc 디렉토리 제외하고 검색하기
# find / -name file1 -not -path '/proc/*'
```

```bash
find 명령어에서 -exec 옵션에서 세미콜론(;)의 역할
find 명령어를 사용하면서 출력된 결과를 가지고 바로 후처리 실행으로 이어질수 있는 옵션인 exec ok가 존재한다. 해당 문법의마지막에는 세미콜론(;)이 입력된다. 하지만 쉘에서는 세미콜론(;)을 기준으로 앞 명령어와 뒤 명령어를 구분하는 구분자로 사용하기 때문에 그냥 사용할수는 없으며 바로 앞에 역슬래시를 붙여 \; 구조로 만들어줘야 find 명령어에서 정상적으로 해석가능하다.
{} -> find된 결과 값들
```

(실무 예) 오래된 로그 기록 삭제

```bash
한달(시스템 생성일 ~ 30일)이 지난 로그파일은 그 의미를 상실하게 된다. 따라서 일정 시간이 지난 로그파일의 경우 find라는 명령어를 이용하여 파일을 주기적으로 삭제해 주도록 한다.

# find /Log_Dir1 -name "*.log" -type f -mtime +30 -exec rm –f {} \;
# find /Log_dir2 -name "*.log" -type f -mtime +60 -exec rm –f {} \;
```

(실무 예) 파일시스템이 갑자기 풀(Full) 나는 경우

```bash
# find /var -mtime -2 -size +1G –type f
# find /var -mtime -2 -size +512M -type f
/var/server/log/file.log

[참고] lsof(list open file)
# lsof | grep /var/server/log/file.log
```

(실무 예) 에러메세지가 들어 있는 startup script 검색

```bash
# /was/bin/startup.sh 
..... Server Error .....
# find /was –type f –exec grep –l 'Server Error' {} \;
/was/conf/server.xml
# vi /was/conf/server.xml
/Server Error
......
if 조건 ; then

else
echo "Server Error"
fi
```

(실무 예) 에러 메세지를 검색하는 방법

```bash
http://www.google.co.kr
-> site:.redhat.com "Server Error"
-> 가상화 .pdf
-> 가상화 .ppt
-> "Server Error1" AND "Server Error2" (AND/OR)
-> "Server Error1"
```

## 문제 풀기

```bash
server1을 초기화한다.
네트워크 상태를 확인

선수작업
디렉토리 생성
/test
/goott
/training
계정생성
fedora

1. bash를 3개 실행하여 ps로 확인했을때
--------------------------------
# ps
   PID TTY          TIME CMD
  1890 pts/0    00:00:00 bash
 58409 pts/0    00:00:00 bash
 58854 pts/0    00:00:00 bash
 58889 pts/0    00:00:00 ps
--------------------------------
해당 화면과 비슷하게 나와야 한다.

2. 아래 해당하는 명령어를 echo 명령어로 출력하세요.  
시스템을 15분뒤 종료하는 명령어를 실행시키고 실제로는 종료되지 못하도록 취소한다.

3. 아래 해당하는 명령어를 echo 명령어로 출력하세요. 
tree 명령어를 사용가능한 상태로 시스템에 프로그램을 설치한다.

* 4번 실습전 자신의 전용 계정을 하나 생성합니다.
이후 생성한 디렉토리로 이동하여 디렉토리의 위치를 확인한다.
디렉토리의 권한을 검색 및 확인하고 생성한 디렉토리로 이동합니다. 
이동후 디렉토리 하위에 training 이라는 디렉토리를 하나 생성하고
		system_vol 이라는 일반파일을 하나 생성합니다.

4. ls 명령어의 실제 경로의 출력물을 해당 디렉토리의 ls_path 파일로 저장하고 해당 파일의 형식을 확인한뒤 파일의 생성시간을 확인하고 
저장된 파일의 텍스트를 확인한다.

5. tree 명령어의 실제경로와 도움말 페이지의 실제 경로의 출력물 tree_path 파일로 저장하고 해당 파일의 형식을 확인한뒤 파일의 
생성시간을 확인하고 저장된 파일의 텍스트를 확인한다. 
해당 파일의 inode를 확인하고 이후 해당 파일의 이름을 tree로 변경한다.
다시 한번 inode 번호를 확인하여 두 inode 번호가 동일해야 한다.
이후 동일한 inode를 가지는 tree2 라는 파일을 생성한다.
tree2파일의 경로를 링크로 가지는 tree3  이라는 파일을 생성한뒤 확인한다.

6. 시스템의 호스트 이름을 hostnamecheck 파일로 저장한다.

7. 시스템의 버전 정보를 hostnamecheck 파일에 이어 붙인다.

8. 아래 해당하는 명령어를 echo 명령어로 출력하세요.
시스템의 시간을 2005년 09월 15일 17시23분으로 변경한뒤 확인한다.
   time.bora.net 을 이용하여 시간서버와 내 시스템의 시간을 동기화 한다.
Hint : rdate 

9. /etc/resolv.conf 파일의 내용을 resolv_[오늘날짜] 파일로 생성한다. [오늘날짜]는 시간 출력 형식 포맷을 변경하여 구성한다.

10. resolv_[오늘날짜]의 파일을 해당 디렉토리의 숨김파일로 변경한다.

11. 홈 디렉토리로 이동한다.
    이전 디렉토리로 다시 이동한다.

12. 홈 디렉토리로 이동하는데 상대경로를 이용한다.

13. 홈 디렉토리 내에서 /[권한이 존재하는 디렉토리] 내에 디렉토리와 파일을 생성한다.
	* 조건 : 상대경로를 이용한다.
/[개인계정 디렉토리]
`-- dir1
    |-- dir2
    |   `-- dir3
    |       `-- file1
    `-- file2

해당 tree 명령어의 결과처럼 구성되어야 한다.

14. /[개인계정 디렉토리] 디렉토리로 이동한다. /[개인계정 디렉토리] 디렉토리내의 파일과 디렉토리를 삭제한다.
다음 명령어를 입력한다.
# cp /etc/passwd file1
# ln -s file1 file2
# cp /bin/ls file3
# cp /etc/passwd .file4
# mkdir dir1
# cp -r /etc/sysconfig dir2

해당 파일들의 파일 종류를 확인한다.

15. /etc/services 파일을 file1에 여러번 붙여넣어 큰 파일을 생성하고 사람이 보았을대 용량을 보기 좋은 옵션으로 변환하여 
출력한다.

16. file1의 접근시간 수정시간을 아래 출력 결과처럼 변경한다.

Access: 2019-08-30 13:00:00.000000000
Modify: 2019-08-30 13:00:00.000000000

17. file1의 속성정보 변경하기
해당 명령어를 입력한다.
# chmod 777 file1
# chown :fedora file1

속성정보를 유지한채로 페도라계정내로 복사한다. 복사되는 파일이름은 fedora이다.

18. /etc/sysconfig 디렉토리를 team01 디렉토리로 복사한다.
복사시 /home/team01을 사용하지 말고 다른 방법을 이용한다.

19. /test/file1의 용량을 0으로 변경한다.

20. dir2의 모든 파일을 대상으로 한다. 
dir2의 일반 파일 개수를 확인한다.
dir2의 디렉토리 개수를 확인한다.
dir2 내의 링크파일을 확인한다.
```

내가 푼 문제 풀이

```bash
[root@server1 /]# ls -l | grep -E 'test|goott|training'
drwxr-xr-x.   2 root root    6 12월  7 15:46 goott
drwxr-xr-x.   2 root root    6 12월  7 15:46 test
drwxr-xr-x.   2 root root    6 12월  7 15:47 training
1.
# bash
# bash
# ps
    PID TTY          TIME CMD
   2550 pts/0    00:00:00 bash
   2792 pts/0    00:00:00 bash
   2827 pts/0    00:00:00 bash
   2858 pts/0    00:00:00 ps

2.
# echo 'shutdown -h +15; shutdown -c'
shutdown -h +15; shutdown -c

3.
# echo 'dnf -y install tree'
yum install tree

4번 전 선수작업.
# cd ~sk
# pwd
/home/sk
# ls -ld .
drwx------. 3 sk sk 78 12월  7  2020 .

4.
# which ls | tee ls_path
# ls -l ls_path
-rw-r--r--. 1 root root 40 12월  7 16:01 ls_path
# cat ls_path
alias ls='ls --color=auto'
	/usr/bin/ls
# which tree | tee tree_path; which man | tee -a tree_path
# ls -li tree_path
100673260 -rw-r--r--. 1 root root 27 12월  7 16:04 tree_path
# ln tree tree2
# ln -s tree2 tree3
# ls -li | grep 'tree*'
100673260 -rw-r--r--. 2 root root 27 12월  7 16:04 tree
100673260 -rw-r--r--. 2 root root 27 12월  7 16:04 tree2
100673272 lrwxrwxrwx. 1 root root  5 12월  7 16:08 tree3 -> tree2
# hostname | tee hostnamecheck
# cat /etc/redhat-release | tee -a hostnamecheck
# date -s "20050915 17:23:00"
2005. 09. 15. (목) 17:23:00 KST
# rdate -s time.bora.net
# echo 'date -s "20050915 17:23:00"; rdate -s time.bora.net'
# cp /etc/resolv.conf resolv_$(date '+%Y-%m-%d')
# ls -l re*
-rw-r--r--. 1 root root 73  9월 15 17:28 resolv_2005-09-15
# mv resolv_2005-09-15 .resolv_2005-09-15
[root@server1 training]# ls -la
합계 20
drwxr-xr-x.  2 root root 124  9월 15 17:30 .
dr-xr-xr-x. 20 root root 265 12월  7  2020 ..
-rw-r--r--.  1 root root  73  9월 15 17:28 .resolv_2005-09-15
# cd /home
# cd ..
# mkdir dir1
# mkdir dir1/dir2
# mkdir dir1/dir2/dir3
# touch dir1/dir2/dir3/file1
# touch file2
# tree
.
├── dir1
│   └── dir2
│       └── dir3
│           └── file1
└── file2
# ls -al
합계 276
drwxr-xr-x.  4 root root     83  9월 15 17:35 .
dr-xr-xr-x. 21 root root    275  9월 15 17:33 ..
-rw-r--r--.  1 root root   2742  9월 15 17:35 .file4
drwxr-xr-x.  2 root root      6  9월 15 17:35 dir1
drwxr-xr-x.  7 root root   4096  9월 15 17:35 dir2
-rw-r--r--.  1 root root   2742  9월 15 17:34 file1
lrwxrwxrwx.  1 root root      5  9월 15 17:35 file2 -> file1
-rwxr-xr-x.  1 root root 267000  9월 15 17:35 file3
# cat /etc/services | tee -a file1
# ls -lh file1
-rw-r--r--. 1 root root 1.4M  9월 15 17:37 file1
# touch -t 08301300 file1
# stat file1
  File: file1
  Size: 1387246   	Blocks: 2712       IO Block: 4096   일반 파일
Device: fd00h/64768d	Inode: 3043877     Links: 1
Access: (0644/-rw-r--r--)  Uid: (    0/    root)   Gid: (    0/    root)
Context: unconfined_u:object_r:default_t:s0
Access: 2005-08-30 13:00:00.000000000 +0900
Modify: 2005-08-30 13:00:00.000000000 +0900
# cp -p file1 /home/fedora/fedora
# ls -l file1
-rwxrwxrwx. 1 root fedora 1387246  8월 30 13:00 file1
# cd /home/fedora
# ls -l fedora
-rwxrwxrwx. 1 root fedora 1387246  8월 30 13:00 fedora
# cd /home/team01
# cp -r /etc/sysconfig .
# ls
sysconfig
# ls -lh file1
-rwxrwxrwx. 1 root fedora 1.4M  8월 30 13:00 file1
# cat /dev/null > file1
# ls -lh file1
-rwxrwxrwx. 1 root fedora 0  9월 15 17:50 file1
[root@server1 dir2]# find . -type f | wc -l
34
[root@server1 dir2]# find . -type d | wc -l
10
[root@server1 dir2]# find . -type l | wc -l
2
```

강사님의 문제 풀이

```bash
server1을 초기화한다.
네트워크 상태를 확인
# ping www.google.com
선수작업
디렉토리 생성
/test
/goott
/training
[root@server1 /]# ls -l | grep -E 'test|goott|training'
drwxr-xr-x.   2 root root    6 12월  7 15:46 goott
drwxr-xr-x.   2 root root    6 12월  7 15:46 test
drwxr-xr-x.   2 root root    6 12월  7 15:47 training
계정생성
fedora
# useradd fedora
# echo fedora | passwd -stdin fedora

1. bash를 3개 실행하여 ps로 확인했을때
--------------------------------
# ps
   PID TTY          TIME CMD
  1890 pts/0    00:00:00 bash
 58409 pts/0    00:00:00 bash
 58854 pts/0    00:00:00 bash
 58889 pts/0    00:00:00 ps
--------------------------------
해당 화면과 비슷하게 나와야 한다.
# bash
# bash
# ps

2. 아래 해당하는 명령어를 echo 명령어로 출력하세요.  
시스템을 15분뒤 종료하는 명령어를 실행시키고 실제로는 종료되지 못하도록 취소한다.
# echo 'shutdown -h +15'
shutdown -h +15
# echo 'shutdown -c'
shutdown -c

3. 아래 해당하는 명령어를 echo 명령어로 출력하세요. 
tree 명령어를 사용가능한 상태로 시스템에 프로그램을 설치한다.
# echo 'dnf -y install tree'
dnf -y install tree

* 4번 실습전 자신의 전용 계정을 하나 생성합니다.
# useradd sk
# echo sk | passwd --stdin sk 
kiyeon 사용자의 비밀 번호 변경 중
passwd: 모든 인증 토큰이 성공적으로 업데이트 되었습니다.
이후 생성한 디렉토리로 이동하여 디렉토리의 위치를 확인한다.
# cd ~sk
# pwd
/home/sk
디렉토리의 권한을 검색 및 확인하고 생성한 디렉토리로 이동합니다. 
# ls -ld .
drwx------. 3 sk sk 78 12월  8 02:52 .
이동후 디렉토리 하위에 training 이라는 디렉토리를 하나 생성하고
		system_vol 이라는 일반파일을 하나 생성합니다.
# mkdir training
# touch training/system_vol
# tree
.
└── training
    └── system_vol

1 directory, 1 file
# pwd
/home/sk

4. ls 명령어의 실제 경로의 출력물을 해당 디렉토리의 ls_path 파일로 저장하고 해당 파일의 형식을 확인한뒤 파일의 생성시간을 확인하고 
저장된 파일의 텍스트를 확인한다.
# pwd
/home/sk
# which ls > ls_path
# ls
ls_path  training
# file ls_path 
ls_path: ASCII text
# stat ls_path 
  File: ls_path
  Size: 40        	Blocks: 8          IO Block: 4096   일반 파일
Device: fd02h/64770d	Inode: 145         Links: 1
Access: (0644/-rw-r--r--)  Uid: (    0/    root)   Gid: (    0/    root)
Context: unconfined_u:object_r:user_home_t:s0
Access: 2020-12-08 02:55:49.624157393 +0900
Modify: 2020-12-08 02:55:41.705057216 +0900
Change: 2020-12-08 02:55:41.705057216 +0900
 Birth: -
# cat ls_path 
alias ls='ls --color=auto'
	/usr/bin/ls
	
5. tree 명령어의 실제경로와 도움말 페이지의 실제 경로의 출력물 tree_path 파일로 저장하고 해당 파일의 형식을 확인한뒤 파일의 
생성시간을 확인하고 저장된 파일의 텍스트를 확인한다. 
해당 파일의 inode를 확인하고 이후 해당 파일의 이름을 tree로 변경한다.
다시 한번 inode 번호를 확인하여 두 inode 번호가 동일해야 한다.
이후 동일한 inode를 가지는 tree2 라는 파일을 생성한다.
tree2파일의 경로를 링크로 가지는 tree3  이라는 파일을 생성한뒤 확인한다.
# which tree > tree_path
# whereis tree >> tree_path
# ls
ls_path  training  tree_path
# cat tree_path 
/usr/bin/tree
tree: /usr/bin/tree /usr/share/man/man1/tree.1.gz
# file tree_path 
tree_path: ASCII text
# stat tree_path 
  File: tree_path
  Size: 64        	Blocks: 8          IO Block: 4096   일반 파일
Device: fd02h/64770d	Inode: 146         Links: 1
Access: (0644/-rw-r--r--)  Uid: (    0/    root)   Gid: (    0/    root)
Context: unconfined_u:object_r:user_home_t:s0
Access: 2020-12-08 02:58:30.034761055 +0900
Modify: 2020-12-08 02:58:24.118160927 +0900
Change: 2020-12-08 02:58:24.118160927 +0900
 Birth: -
# ls -li tree_path 
146 -rw-r--r--. 1 root root 64 12월  8 02:58 tree_path
# mv tree_path tree
# ls -li tree
146 -rw-r--r--. 1 root root 64 12월  8 02:58 tree
# ln tree tree2
# ls -li tree tree2
146 -rw-r--r--. 2 root root 64 12월  8 02:58 tree
146 -rw-r--r--. 2 root root 64 12월  8 02:58 tree2
# ln -s tree tree3
# ls -l tree3
lrwxrwxrwx. 1 root root 4 12월  8 02:59 tree3 -> tree

6. 시스템의 호스트 이름을 hostnamecheck 파일로 저장한다.
# hostname > hostnamecheck

7. 시스템의 버전 정보를 hostnamecheck 파일에 이어 붙인다.
# cat /etc/redhat-release >> hostnamecheck

8. 아래 해당하는 명령어를 echo 명령어로 출력하세요.
시스템의 시간을 2005년 09월 15일 17시23분으로 변경한뒤 확인한다.
   time.bora.net 을 이용하여 시간서버와 내 시스템의 시간을 동기화 한다.
Hint : rdate 
# echo 'date 091517232005; rdate -s time.bora.net'

9. /etc/resolv.conf 파일의 내용을 resolv_[오늘날짜] 파일로 생성한다. [오늘날짜]는 시간 출력 형식 포맷을 변경하여 구성한다.
# cat /etc/resolv.conf > resolv_'date +%m%d'

10. resolv_[오늘날짜]의 파일을 해당 디렉토리의 숨김파일로 변경한다.
# mv resolv_1208 .resolv_1208

11. 홈 디렉토리로 이동한다.
    이전 디렉토리로 다시 이동한다.
# cd
# pwd
# cd -
# pwd

12. 홈 디렉토리로 이동하는데 상대경로를 이용한다.
# cd ../../root/
# pwd

13. 홈 디렉토리 내에서 /[권한이 존재하는 디렉토리] 내에 디렉토리와 파일을 생성한다.
	* 조건 : 상대경로를 이용한다.
/[개인계정 디렉토리]
`-- dir1
    |-- dir2
    |   `-- dir3
    |       `-- file1
    `-- file2
해당 tree 명령어의 결과처럼 구성되어야 한다.
# cd 
# mkdir -p dir1/dir2/dir3
# touch dir1/dir2/dir3/file1
# touch dir1/file2
# tree dir1
dir1
├── dir2
│   └── dir3
│       └── file1
└── file2

14. /[개인계정 디렉토리] 디렉토리로 이동한다. /[개인계정 디렉토리] 디렉토리내의 파일과 디렉토리를 삭제한다.
다음 명령어를 입력한다.
# cp /etc/passwd file1
# ln -s file1 file2
# cp /bin/ls file3
# cp /etc/passwd .file4
# mkdir dir1
# cp -r /etc/sysconfig dir2
해당 파일들의 파일 종류를 확인한다.
# file *

15. /etc/services 파일을 file1에 여러번 붙여넣어 큰 파일을 생성하고 사람이 보았을대 용량을 보기 좋은 옵션으로 변환하여 
출력한다.
# cat /etc/services >> file1
# !!
# !!
# ls -lh file1
-rw-r--r--. 1 root root 1.4M  9월 15 17:37 file1

16. file1의 접근시간 수정시간을 아래 출력 결과처럼 변경한다.

Access: 2019-08-30 13:00:00.000000000
Modify: 2019-08-30 13:00:00.000000000
# touch -t 201908301300 file1
# stat file1
  File: file1
  Size: 1387246   	Blocks: 2712       IO Block: 4096   일반 파일
Device: fd00h/64768d	Inode: 3043877     Links: 1
Access: (0644/-rw-r--r--)  Uid: (    0/    root)   Gid: (    0/    root)
Context: unconfined_u:object_r:default_t:s0
Access: 2019-08-30 13:00:00.000000000 +0900
Modify: 2019-08-30 13:00:00.000000000 +0900

17. file1의 속성정보 변경하기
해당 명령어를 입력한다.
# chmod 777 file1
# chown :fedora file1
속성정보를 유지한채로 페도라계정내로 복사한다. 복사되는 파일이름은 fedora이다.
# cp -p file /home/fedora/fedora

18. /etc/sysconfig 디렉토리를 team01 디렉토리로 복사한다.
복사시 /home/team01을 사용하지 말고 다른 방법을 이용한다.
# cp -r /etc/sysconfig ~team01

19. /test/file1의 용량을 0으로 변경한다.
# cat /dev/null > file1
# cp /dev/null file1
# > file1

20. dir2의 모든 파일을 대상으로 한다. 
dir2의 일반 파일 개수를 확인한다.
# find dir2 -type f | wc -l
dir2의 디렉토리 개수를 확인한다.
# find dir2 -type d | wc -l
dir2 내의 링크파일을 확인한다.
# find dir2 -type l | wc -l
```
