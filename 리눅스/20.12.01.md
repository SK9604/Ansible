...
- [ ]  파일의 이름 생성 시 규칙
    ...
    - 파일의 이름으로 .(Dot), _(Underbar), -(Dash) 사용가능하다. .(Dot)이 제일 앞에 올 경우 숨김파일이 된다.
...
windows, linux 혼용해서 사용하는 회사, 시스템
...
영어는 문제 없고 한글은 문제가 된다

한글 표준 인코딩이 서로 다르기 때문에 생기는 문제 (조합형, 완성형)
...
```bash
파일 이름  : 디렉토리에서 파일을 구분하는 형태. 
					   디렉토리는 파일을 저장하지 않는다. 
					   파일의 이름과 inode 번호만 저장한다.
```
...
파일의 이름, inode 번호가 저장되어 있다.

기본적으로 `. ..` 이라는 두가지 디렉토리가 존재
...
file2번 내부에는 file1의  inode가 아니라 file1의 path 정보가 저장된다. 경로에 대하여 절대 경로, 상대경로 지정하는 상태로 저장되기 때문에 원본파일을 절대경로로 추가하는 것을 추천한다.
...
# ls -l
	crw-rw----.
	drwxr-xr-x.
	c - 캐릭터 파일 (512 byte)    : tty, tape - LTO, cdrom
	b - 블록 파일 (4K, 4096 byte) : Disk

I/O 단위의 크기
Input/Output 입출력 사이즈
한번의 입출력이 많아야 하는 장치의 경우에는 블록 장치로 동작을 하고 그렇지 않은 장치는 캐릭터로 동작을 한다.
...
..... (중략) .....         ↑     ↑
             Major device 번호   Minor device 번호
```

```bash
# ls -l /dev | grep '^c'    /* 캐릭터 디바이스 */
...
변경 하려는 계정이 존재 하는지 항상 확인

계정 : /etc/passwd

그룹 : /etc/group

두 가지 파일에서 변경하려는 계정이 있는 지 꼭 확인
...
![리눅스 정리 마인드맵](https://user-images.githubusercontent.com/56914461/100725468-6ba01200-3407-11eb-9671-cb3e6a7288a4.png)
