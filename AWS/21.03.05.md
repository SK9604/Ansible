...
윈도우의 인증 DB = SAM (Security Account Manager, 토큰으로 인증한다)

사용자의 비밀번호를 저장하는 데이터베이스 파일

ID, Password 입력시 SAM db에서 자격 증명을 받는다 → 증명이 되면 Access Token을 받는다 → 화면이 열린다
...
- IAM 사용자(UBAC,user base access control), 그룹(GBAC, group ~) 및 역할(RBAC, role ~)
- 연동 자격 증명 관리
- Amazon Cognito (자격증명 풀)
- AWS Organizations (OU, 조직 단위)

## 사용자 계정 및 IAM

### AWS 계정 루트 사용자

이 계정은 모든 AWS 서비스 및 리소스에 대한 전체 액세스 권한을 갖는다.
...
```
루트는 절대 외부 접근을 허용하면 안된다!!
그만큼 위험성이 크다
권장 : 루트 계정을 사용하지 마라 -> 별도의 IAM에게 관리자 access권한을 풀면 된다
정책 중에 AdministratorAccess가 루트에 준하는 권한을 준다.
루트는 MFA(Multi-Factor Authentication)를 이용하여 OTP처럼 인증을 받는다.
```
...
적절한 권한? 최소권한의 원칙 = 업무적으로 필요한 권한만 부여한다.
...
가장 좋은 것은 GBAC(Group Base Access Control) 그룹에 소속되면 그룹에 대한 권한을 상속받는다.

### IAM 보안 주체

보안 주체 = AWS 리소스에서 작업을 수행할 수 있는 entity(최소 단위)
...
자격 증명 공급자(IdP) → SAML(Security Assertion Markup Language), OpenID Connect 등으로 자격증명 추가 할 수 있다.

### IAM 사용자

IAM 사용자는 별도의 AWS계정(root)이 아니라 계정 내 사용자입니다.

각 사용자는 영구적인 자체 자격 증명을 갖고 있는 개별적으로 명명된 작업자입니다.

IAM 사용자는 부여된 권한(정책)을 기준으로 특정 AWS 작업을 수행할 권한이 있습니다.
...
AWS Management Console 또는 CLI에 대한 액세스는 명시적으로 부여되어야 합니다.
...
- IAM 정책은 AWS 서비스에 대한 액세스만 제어합니다.
    ...
    - 예로, EC2를 실행하도록 허용하는 IAM 정책을 사용할 수 있지만 고객이 소유한 인스턴스에 로그인하는 기능을 제어하는 IAM 정책은 없다.
    - IAM은 AWS 프로세스의 일부인 API만 제어
...
정책 생성기를 통해서 Allow/Deny를 JSON 형식으로 준다.
...
### 실습 - 실제로 정책을 확인해보고 적용하기

- 캡처

임의로 퍼블릭 버킷을 만든다.

임의의 파일을 업로드 한 후 퍼블릭으로 설정 한다.

객체 URL에 HTTPS 로 접근하지만 HTTP로도 접근이 가능한 상태이다.

이를 HTTPS만 접근 허용하고 HTTP는 접근 불가하도록 정책을 생성하자.

새로 임의의 파일을 업로드 하고 퍼블릭으로 설정하지 않는다.

버킷의 권한 탭에 버킷 정책 편집 → 정책 생성기
...
버킷 정책 편집의 정책에 붙여넣기 (arn 뒤에 /* 붙이기) → 변경사항 저장

버킷이 퍼블릭 액세스 가능하다

여전히 HTTPS, HTTP 모두 접근이 가능하다

버킷 정책을 복사해 와서 notepad++에서 편집한다.
...
다시 확인해보면 HTTPS는 접근이 되고 HTTP는 접근이 막힌 것을 볼 수 있다.
...
```
# AWS 계정 사용 권고 방안
	ㄴ root 사용자 제한 -> MFA 권한 (google OTP 앱 다운)
	ㄴ UBAC: 관리자 계정(사용자) -> 관리자 권한(정책) 부여
	ㄴ 사용자 계정 관리
			ㄴ GBAC: 그룹 단위 관리 권장
					ㄴ RBAC: Role(역할, 정책)
	: EC2에만! 접근 가능한 IAM 사용자 및 그룹 생성
```
...
- MFA 설정
    ...
    보안 자격 증명 - MFA 활성화
    가상 MFA 디바이스 - 계속
    google otp에서 QR 코드 스캔 후 MFA 코드 연속으로 두번 입력 -> MFA 할당
    완료
    로그아웃 후 재 로그인
    MFA코드 입력해야 한다!
...
- 관리자 권한 계정 생성
    ...
    AWS console - IAM
    사용자 - 사용자 추가
    사용자 이름 : korea_admin, AWS Management Console 액세스, 콘솔 비밀번호: 사용자 지정 비밀 번호 - 다음
    권한 설정 - 그룹 생성
    그룹 이름 : admin_team, 정책: AdministratorAccess(관리자 권한) - 다음
    확인 - 사용자 만들기
    csv 파일 받기
    Console login link의 앞 숫자가 계정 ID(12자리) 이다
    사용자 이름, 암호 입력 후 로그인
    비밀 번호 변경 창
    로그인 됨 확인
...
- 일반 사용자에 특정 권한 부여하기
    ...
    생성 완료
    opec2_user1 클릭
    요약 - 보안 자격 증명 - 액세스 키 - 액세스 키 만들기
    콘솔 로그인 링크 클릭 - opec2_user1로 로그인
    ec2 접속 확인
    다른 서비스(S3)는 권한이 없음을 확인
...
- 개별적으로 각 정책을 주는 방법
    ...
    그룹 op_ec2_acc에 S3, RDS 권한을 추가적으로 주자 그룹에 속한 opec2_user1 계정으로 아까 접근이 안되던 S3에 접근이 되는 것을 확인할 수 있다.

    그러나 S3, RDS, EC2가 아닌 CloudFront에는 접근 할 수 없음을 확인 할 수 있다.
...
사용자는 두 그룹에 속해 두 그룹의 권한을 모두 가질 수 있다.
...
사용자 액세스 자격 증명 브로커 (애플리케이션을 통함) → 자격 증명 브로커 인증 사용자 → 임시 자격증명 요청(AWS STS에서)→ 임시 자격 증명(애플리케이션에 반환)
...
3번의 인증을 거쳐 탄탄한 보안 검증을 받을 수 있다.

[일반적인 Cognito 시나리오](https://docs.aws.amazon.com/cognito/latest/developerguide/cognito-scenarios.html)

### AWS Landing Zone

AWS 모범 사례에 따라 안전한 다중 계정 AWS 환경을 빠르게 설정할 수 있도록 도와주는 솔루션으로 다음 기능을 갖추고 있습니다.

다중 계정 구조, Account Vending Machine, 사용자 액세스, 알림
...
Q: 리소스에 임시 권한을 부여해야 하는 경우 무엇을 사용합니까?

IAM 역할

Q: 하나의 사용자가 S3 버킷에 액세스할 수 없습니다. 문제의 원인을 파악하려면 무엇을 확인해야 합니까?
...
☝, 프로덕션 환경에서 AWS 계정 루트 사용자 사용을 중지합니다!

✌, 가능한 경우 앱이 웹 자격 증명 연동을 통해 IAM 역할을 사용하도록 합니다

# 캐싱

```
cache? 주기억 장치와 보조기억 장치 사이에 속도차를 줄이기 위한 메모리.
        ㄴ 메모리  >  ㄴ 디스크 : 속도가 메모리가 훨씬 빠르다
        ㄴ 실무에서의 캐시는 접근 속도 향상 -> 응답 시간 빠르다. (품질 향상)
            ㄴ 웹
            ㄴ 데이터베이스
            ㄴ 엣지 캐시
	이 속도 차이를 완충시키는 역할
...
수집하려면 느리고 비싼 쿼리가 필요한 대용량 데이터

비교적 정적이고 자주 액세스 하는 데이터(예: 소셜 미디어 웹 사이트의 프로필)
...
중간에 캐시를 달아두면 1회 이상 사용됐던 데이터는 굳이 데이터베이스까지 안가더라도 캐시까지 가고 다시 사용자로 돌아갈 수 있다.
...
기본적으로 CloudFront는 캐시에 콘텐츠를 저장하고 있다.
...
```
DBA..
	가장 많이 하는 업무 : SQL Tuning -> 최적의 SQL을 만든다.
		최적의 SQL? 
			cpu 소비시간 적게 (2순위) < (더 중요) disk I/O량 적게
```
...
점수 같은 것은 실시간으로 바뀐다. 세션을 저장해서 세션 정보를 계속해서 반영해준다는 것.

밀리초 정도 단위로는 실시간에 부족하다.

DynamoDB는 응답시간이 아주 빠르다.
...
주식
	1초당 5만건의 거래가 일어난다.
	캐싱 서버를 둬서 좀 더 빠르게(마이크로초 단위) 처리한다.
```
...
Mem : 20개까지 노드를 확장할 수 있다

Redis : 90개까지 노드를 확장할 수 있다
...
1. 캐시를 못 찾음
2. 데이터베이스에서 원본 데이터를 요청함
3. 원본의 복사본을 캐시에 저장하고 요청한 앱에서 사용
4. 이 후 캐시 적중

### TTL 추가

Elastic Cache가 무제한이 아니기 때문에 TTL을 사용한다.
...
≒ 메모리 재사용 정책 : LRU(Least Recently Used)
...
```
결합 해제 : uncupled
결합 → 종속성을 가짐 → 장애가 생기면 전체적으로 영향을 끼칠 수 있다.
모놀리식 컴퓨팅 서비스 : 결합 해제 -> 종속성 제거 -> 클라우드 네이티브 아키텍처 구현
                          ㄴ 마이크로 서비스
                          ㄴ 서버리스 컴퓨팅
종속성을 없애므로서 장애가 발생했을 때 서비스 장애 요인을 줄일 수 있다.
```
...
앱 티어에 부하가 발생해도 웹 티어에는 아무런 영향을 끼치지 않는다.
...
```
1) 요청 들어옴
2) 웹 티어에서 주문정보를 고객 주문 대기열 넣는다.
	고객 주문 대기열 = SQS 대기열
	FIFO
3) 앱 티어에 대기열 주문정보를 가져간다. → 처리 시작
4) 데이터 처리 진행 → 여러 차례 시도 → 이 과정에서 누락이 발생 할 수 있음
5) 누락된 데이터는 람다함수에 의해 앱 티어나 RDS에서 처리하거나 다시 되돌려준다.
```
...
### Amazon SNS 및 Amazn S3를 이용한 소결합 구현
...
- API 게이트웨이
    ...
    AWS Console - Lambda
    함수 생성 - 새로 작성
    함수 이름 : mylambda, 런타임 Node.js 14.x
    코드소스 확인
    함수 개요 - 트리거 추가
    API 게이트 웨이 - API 생성 - HTTP API - 열기 - 추가
    API 게이트웨이 세부 정보 - API 엔드포인트 주소 복사 - 브라우저 붙여넣기
    mylambda index.js 코드 수정 - Deploy
    브라우저 확인

    - 캡처
- 파이썬 크롤링

    구글 python download 검색 - 접속
    python 3.7.9 버전 다운로드
    default로 설치
    python code 4개 저장
...
class Melon(Base):
    ...
    해당 디렉토리로 이동 후 main.py를 제외한 나머지 소스코드 넣기
    압축하기 ( ‼ **주의** ‼ 압축한 뒤 바로 파일이 있으면 안되고 폴더 안에 파일들이 있는 구조로 압축하기 (람다함수는 디렉토리 단위로 읽기 때문이다))

    AWS Console로 돌아와 함수 생성
    CMD에서 python 버전 확인 -> 3.7.9
    함수 이름: crawlerpy, 런타임: python 3.7 - 생성
    코드 소스에서 labda_function.py에 main.py 소스코드 붙여넣고 Deploy
    Lambda - 계층 - 계층 생성
    이름: crawlerpy-layer, 업로드: 아까 압축했던 파일 업로드, 호환 런타임: python 3.7 - 생성
    Lambda 함수 - crawlerpy 클릭 - Layers 클릭
    계층 - Add a layer
    사용자 지정 계층 - crawlerpy-layer - 버전: 1 - 추가
    Layers (1) 확인
    AWS Lambda에 업데이트된 콘솔을 비활성화 하면 테스트 이벤트 구성이 보인다. 클릭
    이벤트 이름: crawlerevent - 생성
    테스트 버튼 클릭
    성공 로그 (만약 실패시 아래로 스크롤 후 기본 설정 정보 변경(메모리:512, 제한시간:10초))
    성공 로그 세부정보 확인 (멜론의 차트가 크롤링 되었음을 확인!)

    - 캡처

# RTO/RPO 및 백업 복구 설정

RTO = 다운타임이 얼마나 걸릴 것이냐 (downtime)

RPO = 복구 시점에 대한 얘기 (data 손실)
...
### 사용 사례: 게이트웨이 저장 볼륨을 사용한 오프사이트 백업 솔루션
...
- Amazon S3로부터 백업 데이터를 준비합니다.
- 필요한 인프라를 준비합니다.
    - 준비된 AMI, ELB등이 있는 Amazon EC2 인스턴스.
